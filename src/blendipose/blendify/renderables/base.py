    ...
    def _blender_create_colors_nodes(self):
        ...
        if self._colors_metadatas is not None and self._material_instances is not None:
            for material_instance, colors_metadata in zip(self._material_instances, self._colors_metadatas):
                ...
                elif colors_metadata.type in (TextureColors, FileTextureColors, TextureColorsViaTempFile, VideoFileTextureColors):
                    colors_node = blender_material.node_tree.nodes.new('ShaderNodeTexImage')
                    colors_node.image = colors_metadata.texture
                    colors_node.interpolation = colors_metadata.interpolation
                    if colors_metadata.type is VideoFileTextureColors:
                        colors_node.image_user.frame_duration = colors_node.image.frame_duration
                        colors_node.image_user.frame_start = colors_metadata.frame_start + 1
                else:
