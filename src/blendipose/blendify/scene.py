...
class Scene(metaclass=Singleton):
    ...
    def render(
        ...
        aa_filter_width: float = 1.5,
        frame_number: int = None,
        adaptive_sampling_threshold: Optional[float] = None,
        ...
        if save_depth and aa_filter_width != 0:
            warnings.warn(
                "Anti-aliasing filter is enabled. Saved depth will not be exact."
            )
        ...
        if use_denoiser:
            ...
            bpy.context.scene.cycles.denoising_prefilter = (
                "ACCURATE"  # Options: 'FAST', 'ACCURATE'
            )
        ...
        if adaptive_sampling_threshold is not None:
            bpy.context.scene.cycles.use_adaptive_sampling = True
            bpy.context.scene.cycles.adaptive_threshold = adaptive_sampling_threshold
        else:
    ...
    def preview(
        ...
        assert sys.platform.startswith("linux") or sys.platform.startswith(
            "darwin"
        ), "Preview is only supported on Linux and MacOS"
    ...
    def export(
        path: Union[str, Path],
        include_file_textures: bool = True,
        verbose: bool = False,
    ):
